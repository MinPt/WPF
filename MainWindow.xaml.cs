...
namespace WPF_Labs
{
    ...
    public partial class MainWindow
    {
        ...
        {
            bool flightIsCorrect = int.TryParse(flightNum.Text, out flightNumber);
            var i = flightNumber;
            if (!flightIsCorrect || i < 1 || i > 10)
            {
        ...
        private bool SitIsCorrect(out int sitNumber)
        {
            ...
            bool sitIsCorrect = int.TryParse(sitNum.Text, out sitNumber);
            var i = sitNumber;
            var isOcuppied = _clients.FirstOrDefault(c => c.SitNumber == i) != null;

            if ((!sitIsCorrect || isOcuppied) && i < 1 || i > 300)
            {
